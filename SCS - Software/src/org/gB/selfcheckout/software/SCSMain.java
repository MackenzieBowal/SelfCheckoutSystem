...
public class SCSMain {
	 ...
	 * 		The maximum weight that the self checkout station's electronic scale
	 * 		can output (in grams).
	 * @param scaleSensitivity
	 * 		The sensitivity of the self checkout station's electronic scale
	 * 		(in grams).
	 * @throws Exception
	 * 		Thrown if the SelfCheckoutStation instance cannot be obtained.
	...
	public State init(int scaleMaxWeight, int scaleSensitivity) throws Exception {
		...
		final BigDecimal[] coinDenoms = {new BigDecimal(0.05), new BigDecimal(0.1),
				new BigDecimal(0.25), new BigDecimal(1.0), new BigDecimal(2.0)};
